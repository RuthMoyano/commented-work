...
//server should check data before going into database
function checkIsEmailFunc(req, res, next) { //method which checks that the variable needs to have right email format eg @--.com
...
function checkIsAlphanumericFunc(req, res, next) { //method which checks that the variable only contains alphabets and numbers
  const { errorObj } = res.locals;
