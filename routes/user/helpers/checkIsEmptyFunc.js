...
function checkIsEmptyFunc(req, res, next) {
  ...
  const { errorObj } = res.locals; //making an error object to tell user's error

  for (let key in inComingData) {
    if (checkIsEmpty(inComingData[key])) {      //checking length of "value" of the "keys"
      errorObj[key] = `${key} cannot be empty`;  //if any value is empty add this in the errorObject
    }
  }

  //used for loop to remove bunch of code and now keys are dynamic
